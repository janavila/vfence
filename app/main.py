...
app = FastAPI(title="VFence Monitor API", version="1.0.0", lifespan=lifespan)
...
async def health():
    return {
        "status": "ok",
        "gateway": reader.state.status,
        "serial_port": reader.state.serial_port,
    }
...
@app.get("/api/collars")
async def collars():
    return await asyncio.to_thread(database.list_collars)
